...
import { LayoutGrid, Home, ClipboardList, Truck, CreditCard, LogOut, Package, X, Settings, Landmark } from "lucide-react";
import { useAuth } from "../context/AuthContext";

const NAV_ITEMS = [
  { to: "/home", label: "Home", icon: Home, roles: ["EMPLOYEE", "MANAGER", "ADMIN"] },
  { to: "/dashboard", label: "Dashboard", icon: LayoutGrid, roles: ["EMPLOYEE", "MANAGER", "ADMIN"] },
  { to: "/products", label: "Products", icon: Package, roles: ["EMPLOYEE", "MANAGER"] },
  { to: "/orders", label: "My requests", icon: ClipboardList, roles: ["EMPLOYEE", "MANAGER"] },
  { to: "/track", label: "Track order", icon: Truck, roles: ["EMPLOYEE", "MANAGER"] },
  { to: "/payment", label: "Payment", icon: CreditCard, roles: ["ADMIN"] },
  { to: "/management", label: "Management", icon: Settings, roles: ["ADMIN"] },
  { to: "/supplier-orders", label: "Supplier orders", icon: ClipboardList, roles: ["SUPPLIER"] },
  { to: "/supplier-products", label: "My products", icon: Package, roles: ["SUPPLIER"] },
  { to: "/supplier-payments", label: "Payment history", icon: Landmark, roles: ["SUPPLIER"] },
];
...
export default function Sidebar({ mobileOpen = false, onCloseMobile }) {
  ...
  return (
    <>
      ...
      {/* Mobile drawer — slides in over a backdrop, below lg */}
      {mobileOpen && (
